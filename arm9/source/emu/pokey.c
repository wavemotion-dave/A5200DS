 ...
 ***************************************************************************/
unsigned short bPausePokey = 0;
ITCM_CODE void POKEY_Scanline(void) 
{
    extern unsigned short myPokeyBufIdx;
    unsigned short next_pokey_idx = (pokeyBufIdx+1) & (SNDLENGTH-1);
    
    if (bPausePokey) bPausePokey--;
    else
    {
        if (next_pokey_idx == myPokeyBufIdx)
        {
            bPausePokey = SNDLENGTH / 3;
        }
        else
        {
