...
void dsInitScreenMain(void) 
{
    ...
    SetYtrigger(isDSiMode() ? 128:100); //trigger 64 lines before vsync (or 92 lines before for older DS hardware)
...
void LoadFavorites(void)
{
    ...
    FILE *fp = fopen("/data/a5200ds.fav", "rb");
    if (fp)
    {
...
void SaveFavorites(void)
{
    ...
    FILE *fp = fopen("/data/a5200ds.fav", "wb");
    if (fp)
    {
  ...
  for (ucBcl=0;ucBcl<17; ucBcl++) {
    ...
    if (ucGame < counta5200) 
    {
      ...
      else 
      {
        ...
        // Show favorites...
        if (IsFavorite(a5200romlist[ucGame].filename))
        {
            dsPrintValue(0,5+ucBcl,(IsFavorite(a5200romlist[ucGame].filename) == 1) ? 1:0,(char*)"@");
        }
        else
        {
            dsPrintValue(0,5+ucBcl,0,(char*)" ");
        }        
      }
    }
  }
...
unsigned int dsWaitForRom(void) 
{
  ...
  while (!bDone) {
    ...
    if (keysCurrent() & KEY_SELECT)
    {
        ...
        {
            ToggleFavorite(a5200romlist[ucFicAct].filename);
            dsDisplayFiles(firstRomDisplay,romSelected);
            SaveFavorites();
  ...
  while(etatEmu != A5200_QUITSTDS) 
  {
        ...
        {    
          ...
          case A5200_PLAYGAME:
            ...
            if (!full_speed)
            {
                while(ds_vblank_count < (atari_frames+1))
                    ;
            }
